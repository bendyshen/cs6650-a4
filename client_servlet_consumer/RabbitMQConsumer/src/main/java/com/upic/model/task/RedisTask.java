...
public class RedisTask implements Runnable {
  ...
  /**
   * Caches the vertical totals for a given skier for each ski day.
   *
   * @param pipeline a Jedis connection pipeline
   */
  private void insertVerticalBySkierSeasonDay(Pipeline pipeline, String skierID, String seasonID,
      String dayID, long vertical) {
    String key = "skier:" + skierID + ":season:" + seasonID + ":day:" + dayID + ":vertical";
    pipeline.hincrBy(key, "vertical", vertical);
  }

  /**
   * Caches the vertical totals for a given skier.
   *
   * @param pipeline a Jedis connection pipeline
   */
  private void insertVerticalBySkier(Pipeline pipeline, String skierID, long vertical) {
    String key = "skier:" + skierID + ":vertical";
    pipeline.hincrBy(key, "vertical", vertical);
  }

  /**
   * Caches the vertical totals for a given skier of a given season.
   *
   * @param pipeline a Jedis connection pipeline
   */
  private void insertVerticalBySkierSeason(Pipeline pipeline, String skierID, String seasonID,
      long vertical) {
    String key = "skier:" + skierID + ":season:" + seasonID + ":vertical";
    pipeline.hincrBy(key, "vertical", vertical);
  }
  ...
  /**
   * Caches the number of unique skiers visited a given resort on a given day.
   *
   * @param pipeline a Jedis connection
   */
    ...
    while (batchSize > 0) {
      try (Jedis jedis = redisPool.borrowChannel()) {
        ...
        if (body != null) {
          ...
          insertVerticalBySkierSeasonDay(pipeline, skierID, seasonID, dayID, vertical);
          ...
          insertVerticalBySkier(pipeline, skierID, vertical);
          // Cache skier vertical by season
          insertVerticalBySkierSeason(pipeline, skierID, seasonID, vertical);
          pipeline.sync();
